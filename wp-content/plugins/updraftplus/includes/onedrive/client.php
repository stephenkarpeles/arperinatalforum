...
class Client {
	...
	public function __construct(array $options = array()) {
		...
		$this->api_url   = 'https://apis.live.net/v5.0/';
		if ($this->use_msgraph_api) {
			$this->new_api_url = 'https://graph.microsoft.com/v1.0/';
			...
			$this->route_prefix = '';
			$this->new_api_url = 'https://api.onedrive.com/v1.0/';
	...
	public function apiGet($path, $options = array(), $new_api = true) {
		$api = ($new_api) ? $this->new_api_url : $this->api_url;
	...
	public function fetchQuota() {
		$drive = $this->apiGet($this->route_prefix.'drive', array(), true);
		return $drive->quota;
	}
	...
	public function fetchRecentDocs() {
		return $this->apiGet($this->route_prefix.'drive/special/recent_docs', array(), true);
	}
	...
	public function fetchShared() {
		return $this->apiGet($this->route_prefix.'drive/special/shared', array(), true);
	}
