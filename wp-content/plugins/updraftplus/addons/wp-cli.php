...
class UpdraftPlus_CLI_Command extends WP_CLI_Command {
	...
	public function backup($args, $assoc_args) {
		...
		$backupnow_files = !empty($only_these_file_entities) ? true : false;
...
if (!is_array($accept)) $accept = array();
		...
		foreach ($backup_history as $key => $backup) {
			...
			$items[] = array(
				'job_id'          => strip_tags($date_label).' ['.$nonce.'] ('.implode(',', $backup['service']).')',
				'nonce'           => $nonce,
				'backup_entities' => $backup_entities,
			);
		}
		// @codingStandardsIgnoreLine
		WP_CLI\Utils\format_items('table', $items, array('job_id', 'nonce', 'backup_entities'));
	}
